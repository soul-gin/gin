...
public class HbaseSharedConnPool implements ConnectionPool<Connection> {
	...
	private static final AtomicReference<HbaseSharedConnPool> HBASE_CONN_POOL = new AtomicReference<HbaseSharedConnPool>();
    ...
    public synchronized static HbaseSharedConnPool getInstance(final Configuration configuration) {
        ...
        if (HBASE_CONN_POOL.get() == null) {
            try {
                HBASE_CONN_POOL.set(new HbaseSharedConnPool(configuration));
